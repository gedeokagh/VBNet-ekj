...
Public Class frmColPay
    ...
    Sub filltiket()
        ...
        da = New MySqlDataAdapter("SELECT 0 as ID, tiketdtl.NoETiket as NoETiket,tiketdtl.NoTiket as NoTiket, tiketdtl.GoDate as DateGo, trip.TripName as GoTrip, rute.RuteName as RuteGo,agent.Agentname as Agent,tiketdtl.guest as Guest, tiketdtl.TotalJual, tiketdtl.ReqCollect, tiketdtl.ReqCollect AS `Request` FROM (((trip INNER JOIN tiketdtl ON trip.TripID = tiketdtl.GoTrip) INNER JOIN rute ON tiketdtl.GoRuteID = rute.RuteID) INNER JOIN agent ON tiketdtl.AgentID = agent.AgentID )  WHERE    (tiketdtl.NoCollect = '' OR ISNULL(tiketdtl.NoCollect))   AND (tiketdtl.NoInv = ''OR ISNULL(tiketdtl.NoInv))   AND (tiketdtl.NoPay = ''OR ISNULL(tiketdtl.NoPay))  and (tiketdtl.ReqCollect>0) AND TIKETDTL.STATUS=1 and " & strgo & " ", conn)
        dt = New DataTable
        da.Fill(dt)
    ...
    Sub filltiketx()
        ...
        Else
            If WHR = "" Then
                WHR = " AND ((tiketdtl.AgentID)='" & cboAgent.SelectedItem("AgentID").ToString & "') "
            Else
        ...
        Else
            If WHR = "" Then
                WHR = " AND ((tiketdtl.Rute)='" & cboAgent.SelectedItem("AgentID").ToString & "') "
            Else
                WHR = WHR & " AND ((tiketdtl.AgentID)='" & cboAgent.SelectedItem("AgentID").ToString & "') "
            End If
        End If
        ...
        Dim da As MySqlDataAdapter
        Dim dt As DataTable
        da = New MySqlDataAdapter("SELECT 0 as ID, tiketdtl.NoETiket as NoETiket,tiketdtl.NoTiket as NoTiket, tiketdtl.GoDate as DateGo, trip.TripName as GoTrip, rute.RuteName as RuteGo,tiketdtl.TotalJual,tiketdtl.ReqCollect FROM (((trip INNER JOIN tiketdtl ON trip.TripID = tiketdtl.GoTrip) INNER JOIN rute ON tiketdtl.GoRuteID = rute.RuteID) )  WHERE  ((tiketdtl.GoDate)='" & Str2Date(txtdate.Text) & "') AND  (tiketdtl.NoCollect = '' OR ISNULL(tiketdtl.NoCollect))   AND (tiketdtl.NoInv = ''OR ISNULL(tiketdtl.NoInv))   AND (tiketdtl.NoPay = ''OR ISNULL(tiketdtl.NoPay))  and (tiketdtl.ReqCollect>0) " & WHR & " ", conn)
        dt = New DataTable
        da.Fill(dt)
        lsData.DataSource = dt
        lsData.AllowUserToAddRows = False
        ...
        For a = 0 To (I - 1)
            ...
            If lsData.Rows(a).Cells(0).Value = True Then
                ...
                Try
                    ...
                    Dim ttlj = lsData.Rows(a).Cells(9).Value
                    Dim ttlc = lsData.Rows(a).Cells(10).Value
                    ...
                    If (ttlc <> 0) Then
                        ...
                        If tsisa <> 0 Then
                            tkomisi = tsisa * -1
                            tsisa = 0
                            stSql = "Update tiketdtl set  NoCollect='" & txtNoBook.Text & "',TglCollect='" & Str2Date(txttgltrans.Text) & "',sisa=" & tsisa & ",komisi=" & tkomisi & ",TotalCollect=" & lsData.Rows(a).Cells(10).Value & " where NoETiket='" & lsData.Rows(a).Cells(2).Value & "'"
                        Else
                            tkomisi = 0
                            tsisa = tsisa

                            stSql = "Update tiketdtl set  NoCollect='" & txtNoBook.Text & "',TglCollect='" & Str2Date(txttgltrans.Text) & "',NopAY='" & txtNoBook.Text & "',Tgllunas='" & Str2Date(txtdate.Text) & "',pay=1,sisa=" & tsisa & ",komisi=" & tkomisi & ",TotalCollect=" & lsData.Rows(a).Cells(10).Value & " where NoETiket='" & lsData.Rows(a).Cells(2).Value & "'"
                        End If
                    Else
                        ttl = lsData.Rows(a).Cells(10).Value
                        tsisa = ttlj - lsData.Rows(a).Cells(10).Value
                        If tsisa < 0 Then
                            tkomisi = tsisa * -1
                            tsisa = 0
                            stSql = "Update tiketdtl set  NoCollect='" & txtNoBook.Text & "',TglCollect='" & Str2Date(txttgltrans.Text) & "',sisa=" & tsisa & ",komisi=" & tkomisi & ",TotalCollect=" & lsData.Rows(a).Cells(10).Value & " where NoETiket='" & lsData.Rows(a).Cells(2).Value & "'"
                    ...
                    'stSql = "Update tiketdtl set  NoCollect='" & txtNoBook.Text & "',TglCollect='" & Str2Date(txtdate.Text) & "',sisa=" & tsisa & ",komisi=" & tkomisi & ",TotalCollect=" & lsData.Rows(a).Cells(9).Value & " where NoETiket='" & lsData.Rows(a).Cells(2).Value & "'"
